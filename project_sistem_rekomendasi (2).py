...
- Melihat informasi struktur dataset (jumlah baris, kolom, tipe data, missing value,Duplikat).
...
print("Jumlah duplikat berdasarkan judul di movies_df:", movies_df.duplicated(subset=['title']).sum())

"""Insight :
Terdapat 3 duplikat di kolom title di data set movies
"""

print("Jumlah duplikat berdasarkan judul di credits:", credits_df.duplicated(subset=['title']).sum())

"""Insight :
Terdapat 3 duplikat di kolom title di data set credits
"""
...
"""**Hasil Penanganan Missing Value & duplikat:**

Missing values pada kolom 'homepage', 'overview', 'tagline', 'release_date', dan 'runtime' telah ditangani sesuai strategi yang ditentukan dan juga duplikat pada kolom title sudah di tangani.
...
# Hapus duplikat berdasarkan judul film
movie_data.drop_duplicates(subset=['title'], keep='first', inplace=True)
print("✅ Duplikat berdasarkan judul film telah dibersihkan.\n")
...
"""**Hasil Rekomendasi:**
Untuk film "The Dark Knight", sistem merekomendasikan film-film Batman lainnya seperti "The Dark Knight Rises", "Batman Returns", "Batman Begins", "Batman: The Dark Knight Returns, Part 2", dan "Batman Forever".

Skor similarity berkisar antara 0.3015 hingga 0.4567. Rata-rata skor similarity adalah 0.3630, yang berdasarkan threshold sederhana (< 0.5) dianggap kurang relevan.

Ini menunjukkan bahwa meskipun rekomendasi secara tematik mirip, skor similarity kontennya tidak terlalu tinggi, kemungkinan karena perbedaan dalam detail seperti overview, cast, atau crew yang digunakan sebagai fitur dalam sistem rekomendasi.
